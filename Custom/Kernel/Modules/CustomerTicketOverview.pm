...
sub Run {
    ...
    if ( !$AllTicketsTotal ) {
        ...
        # TTO Customization: Added $Changed
        my $Changed = $ConfigObject->Get('Ticket::Frontend::CustomerTicketOverview')->{Changed};
        ...
        if ($Changed) {
            $LayoutObject->Block(
                Name => 'OverviewNavBarPageChanged',
                Data => {
                    OrderBy     => $OrderBy,
